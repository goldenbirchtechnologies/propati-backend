...
CREATE TABLE IF NOT EXISTS users (
  ...
  agent_approved INTEGER DEFAULT 0,
  agent_bio   TEXT,
  agent_areas TEXT, -- JSON array of areas
...
CREATE TABLE IF NOT EXISTS listings (
  ...
  -- Availability
  available_from  TEXT,
  minimum_stay    INTEGER, -- days (for short-let)

  -- Verification badge (computed from verification table)
...
CREATE TABLE IF NOT EXISTS verifications (
  ...
  l1_doc_coo      TEXT,  -- Certificate of Occupancy URL
  l1_doc_title    TEXT,  -- Title Deed URL
  l1_doc_survey   TEXT,  -- Survey Plan URL
  l1_doc_deed     TEXT,  -- Deed of Assignment URL
  l1_doc_govcon   TEXT,  -- Governor's Consent URL
  l1_submitted_at TEXT,
  l1_reviewed_at  TEXT,
...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  platform_fee      REAL NOT NULL,        -- PROPATI's cut
  agent_commission  REAL DEFAULT 0,
  payee_amount      REAL NOT NULL,        -- what landlord/seller gets

  paystack_ref      TEXT,
  paystack_txn_id   TEXT,

  -- Escrow
  escrow_release_date TEXT,  -- when funds auto-release
...
CREATE TABLE IF NOT EXISTS agreements (
  ...
  -- Document
  doc_url            TEXT, -- generated PDF
  template_vars      TEXT, -- JSON — filled template fields
...
-- Rent schedule (for active agreements)
...
-- ─────────────────────────────────────
-- FLAGS (community flagging system)
-- ─────────────────────────────────────
